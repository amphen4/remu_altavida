                ...
                <form class="form-horizontal" enctype="multipart/form-data" action="" id="formularioUpdate" method="POST" >
		            ...
		            <div class="box-body">
			            ...
			            <div class="row">
			                ...
			                <div class="col-lg-4">
			                    <div class="input-group">
			                          <label >Previsión:</label><label><p style="color:red">*</p></label>
			                ...
			                <div class="col-lg-4">
			                    <div class="input-group">
			                          <label >Cotización Pactada (UF):</label><label><p style="color:red">*</p></label>
			                        <input type="number" step="0.005"   class="form-control" id="editarCotizacionPactada" name="cotizacion_pactada" >
      ...
      <div class="modal-body">
      	<form class="form-horizontal" id="formulario" method="POST" action="{{url('/empleados/create')}}">
		                ...
		                <div class="input-group">
	                        <label >Banco:</label>
                  			<select class="form-control " name="nombre_banco" >
		                  				...
		                  				<option>Banco Edwards Cities</option>
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <div class="input-group">
                          <label >Cotizacion Pactada (UF):</label>
                        <input class="form-control " type="number" min="0" step="0.001" id="cotizacion_pactada"  name="cotizacion_pactada" >
...
<script>
	$(document).ready(function(){
		...
		$('#editarIsapre').change(function(){
			console.log('ha seleccionado '+$("#editarIsapre option:selected").text());

			if($("#editarIsapre option:selected").text() == 'FONASA'){
				$('#editarCotizacionPactada').prop('readonly', true);
				$('#editarCotizacionPactada').prop('disabled', true);
				$('#editarCotizacionPactada').prop('required', false);
			}else{
				$('#editarCotizacionPactada').prop('readonly', false);
				$('#editarCotizacionPactada').prop('disabled', false);
				$('#editarCotizacionPactada').prop('required', true);
			}
		});
    moment.locale('es');
		$('#divLoading').hide();
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        for(var i=0; i<datos.length; i++){
          if(i==0){
            $('#selectIsapre').append(
                '<option selected value="'+datos[i].id+'">'+datos[i].nombre+'</option>'
              );
            $('#editarIsapre').append(
                '<option selected value="'+datos[i].id+'">'+datos[i].nombre+'</option>'
              );
      ...
      function cargarPerfil(id){
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            if(datos.isapre_nombre == 'FONASA'){
            	$('#editarCotizacionPactada').prop('readonly', true);
            	$('#editarCotizacionPactada').prop('disabled', true);
            	$('#editarCotizacionPactada').prop('required', false);
            }else{
            	$('#editarCotizacionPactada').val(datos.cotizacion_pactada);
            	$('#editarCotizacionPactada').prop('readonly', false);
            	$('#editarCotizacionPactada').prop('disabled', false);
            	$('#editarCotizacionPactada').prop('required', true);
            }
